...
def create_nick_live(
    ...
    # Enforce quota
    if ctx_user.max_nicks is not None:
        n = db.query(NickLive).filter(NickLive.user_id == ctx_user.id).count()
        if n >= ctx_user.max_nicks:
            raise HTTPException(
                status_code=403,
                detail=f"Nick quota exceeded (max {ctx_user.max_nicks})",
            )
...
def list_nick_lives(
    ...
    return (
        db.query(NickLive)
        .filter(NickLive.user_id == ctx_user.id)
...
def get_nick_cookies(
    ...
    nick = _owned_nick_or_404(db, nick_live_id, ctx_user.id)
...
async def list_sessions_batch(
    ...
    owned = (
        db.query(NickLive)
        .filter(NickLive.id.in_(nick_ids), NickLive.user_id == ctx_user.id)
        .all()
    )
...
def stop_scan(
    ...
    _owned_nick_or_404(db, nick_live_id, ctx_user.id)
    scanner.stop(nick_live_id)
